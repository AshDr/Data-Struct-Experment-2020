...
void hieOrder(csNode *T) {
	...
	while(!q.empty()) {
		...
		cout<<temp->data;
		q.pop();
		if(temp->firstChild!=NULL) q.push(temp->firstChild);
...
int nodeNum(csNode *T) {
	...
	int ans=1;
	ans+=nodeNum(T->firstChild);
	ans+=nodeNum(T->nextSibling);
	return ans;
}
//叶子节点数 
void leafNum(csNode *T, int &num) {
	if(T==NULL) return;	
	if(T->firstChild==NULL) num++;
	leafNum(T->firstChild,num);
	leafNum(T->nextSibling,num);
	return ;
}
//树的高 
int height(csNode *T) {
